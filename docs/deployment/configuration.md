...
| EnableForkAndExitLogging          | Bool       | If true, Santa will log FORK and EXIT event types. Defaults to NO. |
| IgnoreOtherEndpointSecurityClients | Bool      | If true, Santa will not process events that are generated by other EndpointSecurity clients that may be installed on the system. Defaults to NO. |
