...
| FileAccessPolicyUpdateIntervalSec  | Integer    | Number of seconds between re-reading the file access policy config and updating policies/monitored paths. Minimum value is 15 seconds. This value is only read once when santad starts. Defaults to 600 seconds (10 minutes). |
