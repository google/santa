...
- (NSDictionary *)fixUpDatesAndDataValuesOf:(NSDictionary *)exportedMetrics {
    ...
    NSMutableDictionary *mutableMetrics = [[SNTMetricFormatTestHelper convertDatesToFixedDateWithExportDict:exportedMetrics] mutableCopy];
   
    // Ensure that we got data of the correct type but do not check the values as they'll change per call. Instead we stub out the values for testing.
    NSMutableDictionary *metric = mutableMetrics[@"metrics"][@"/proc/birth_timestamp"];
    XCTAssertTrue([metric[@"fields"][@""][0][@"data"] isKindOfClass:[NSNumber class]], @"/proc/birth_timestamp data is not a number");
    ...
    XCTAssertTrue([metric[@"fields"][@"mode"][0][@"data"] isKindOfClass:[@0.63002 class]], @"/proc/cpu_usage has non-floating point data");
    metric[@"fields"][@"mode"][0][@"data"] = @0.63002;
    XCTAssertTrue([metric[@"fields"][@"mode"][1][@"data"] isKindOfClass:[@0.29522 class]], @"/proc/cpu_usage has non-floating point data");
...
- (void)testRegisteringCoreMetrics {
    ...
    NSString *shortOSVersion = [NSString stringWithFormat:@"%ld.%ld.%ld",
    ...
    XCTAssertEqualObjects(expected[@"root_labels"], exportedMetrics[@"root_labels"], @"root_labels are different");
    ...
    for (NSString *metricName in expected[@"metrics"]) {
        ...
        XCTAssertEqualObjects(expected[@"metrics"][metricName], other, @"%@ does not match expected values", metricName);
    }
}
