...
SNTStoredEvent* createTestBundleStoredEvent(NSBundle *bundle,
    NSString *fakeBundleHash, NSString *fakeFileHash)
{
  if (!bundle) {
    return nil;
  }
...
{
  if (santaMsg.messageTypeOneOfCase != 
      SNTSantaMessage_MessageType_OneOfCase_LegacyMessage) {
  ...
  XCTAssertTrue([event isKindOfClass:expectedClass],
      "Extracted unexpected class");
...
NSBundle* getBundleForSystemApplication(NSString *appName)
{
  if (@available(macOS 10.15, *)) {
    return [NSBundle bundleWithPath:
        [NSString stringWithFormat:@"/System/Applications/%@", appName]];
  } else {
    return [NSBundle bundleWithPath:
        [NSString stringWithFormat:@"/Applications/%@", appName]];
  }
}
...
{
  NSLog(@"p: %d/%d, pv: %d/%d, pp: %d/%d, u: %d/%d, g: %d/%d, un: %@/%@, gn: %@/%@",
      procInfo.pid, expected->pid,
      procInfo.pidversion, expected->pidversion,
      procInfo.ppid, expected->ppid,
      procInfo.uid, expected->uid,
      procInfo.gid, expected->gid,
      procInfo.user, @(user_from_uid(expected->uid, 0)),
      procInfo.group, @(group_from_gid(expected->gid, 0)));
  XCTAssertTrue(
      procInfo.pid == expected->pid &&
      procInfo.pidversion == expected->pidversion &&
      procInfo.ppid == expected->ppid &&
...
- (void)testLogFileModification {
  ...
  OCMExpect([self.mockLogOutput logEvent:[OCMArg checkWithBlock:^BOOL(SNTSantaMessage *sm) {
    SNTLegacyFileModification *fileMod = getEventForMessage(sm,
          SNTLegacyMessage_LegacyMessageType_OneOfCase_FileModification, @"fileModification",
          [SNTLegacyFileModification class]);
    ...
    if (fileMod.action != SNTLegacyFileModification_Action_LegacyFileModificationActionRename ||
        ...
        !fileMod.hasProcessInfo ||
        fileMod.processInfo == nil ||
...
- (void)testLogExecutionDenied {
  ...
  OCMExpect([self.mockLogOutput logEvent:[OCMArg checkWithBlock:^BOOL(SNTSantaMessage *sm) {
    SNTLegacyExecution *exec = getEventForMessage(sm,
          SNTLegacyMessage_LegacyMessageType_OneOfCase_Execution, @"execution",
    ...
    if (exec.decision != SNTLegacyExecution_Decision_LegacyExecutionDecisionDeny ||
        ...
        ![exec.explain isEqualToString:explanation] ||
        ![exec.sha256 isEqualToString:sha256] ||
        ![exec.certSha256 isEqualToString:certSHA256] ||
        ![exec.certCn isEqualToString:commonName] ||
        ![exec.quarantineURL isEqualToString:quarantineURL] ||
        !exec.hasProcessInfo ||
        exec.processInfo == nil ||
        exec.mode != SNTLegacyExecution_Mode_LegacyExecutionModeLockdown ||
        ![exec.path isEqualToString:processPath] ||
        [exec.originalPath length] != 0 ||
        ![exec.argsArray isEqualToArray:execArgs] ||
        [exec.machineId length] != 0) {
...
- (void)testLogDiskAppeared {
  ...
  OCMExpect([self.mockLogOutput logEvent:[OCMArg checkWithBlock:^BOOL(SNTSantaMessage *sm) {
    SNTLegacyDiskAppeared *diskAppeared = getEventForMessage(sm,
          SNTLegacyMessage_LegacyMessageType_OneOfCase_DiskAppeared, @"diskAppeared",
          [SNTLegacyDiskAppeared class]);
    ...
    if (![diskAppeared.mount isEqualToString:mount] ||
        ...
        ![diskAppeared.model isEqualToString:
            [NSString stringWithFormat:@"%@ %@", deviceVendor, deviceModel]] ||
        ![diskAppeared.serial isEqualToString:serial] ||
        ![diskAppeared.bus isEqualToString:deviceProto] ||
        [diskAppeared.dmgPath length] != 0 ||
...
- (void)testLogDiskDisappeared {
  ...
  OCMExpect([self.mockLogOutput logEvent:[OCMArg checkWithBlock:^BOOL(SNTSantaMessage *sm) {
    SNTLegacyDiskDisappeared *diskDisappeared = getEventForMessage(sm,
...
- (void)testLogBundleHashingEvents {
  NSArray<SNTStoredEvent*> *storedEvents = @[
      createTestBundleStoredEvent(getBundleForSystemApplication(@"Calculator.app"),
          @"abc123", @"xyz456"),
      createTestBundleStoredEvent(getBundleForSystemApplication(@"Calendar.app"),
          @"123abc", @"456xyz"),
  ];

  for (SNTStoredEvent *storedEvent in storedEvents) {
    OCMExpect([self.mockLogOutput logEvent:[OCMArg checkWithBlock:^BOOL(SNTSantaMessage *sm) {
      SNTLegacyBundle *bundleEvent = getEventForMessage(sm,
          SNTLegacyMessage_LegacyMessageType_OneOfCase_Bundle, @"bundle",
...
- (void)testLogFork {
  ...
  OCMExpect([self.mockLogOutput logEvent:[OCMArg checkWithBlock:^BOOL(SNTSantaMessage *sm) {
    SNTLegacyFork *forkEvent = getEventForMessage(sm,
          SNTLegacyMessage_LegacyMessageType_OneOfCase_Fork, @"fork",
          [SNTLegacyFork class]);
...
- (void)testLogExit {
  ...
  OCMExpect([self.mockLogOutput logEvent:[OCMArg checkWithBlock:^BOOL(SNTSantaMessage *sm) {
    SNTLegacyExit *exitEvent = getEventForMessage(sm,
          SNTLegacyMessage_LegacyMessageType_OneOfCase_Exit, @"exit",
          [SNTLegacyExit class]);
...
- (void)testLogAllowlist {
  SNTAllowlistInfo *allowlistInfo =
      [[SNTAllowlistInfo alloc] initWithPid:123
                                 pidversion:456
                                 targetPath:@"/sbin/launchd"
                                     sha256:@"abc123"];

  OCMExpect([self.mockLogOutput logEvent:[OCMArg checkWithBlock:^BOOL(SNTSantaMessage *sm) {
    SNTLegacyAllowlist *allowlistEvent = getEventForMessage(sm,
          SNTLegacyMessage_LegacyMessageType_OneOfCase_Allowlist, @"allowlist",
